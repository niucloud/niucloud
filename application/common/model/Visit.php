...
class Visit
{
	...
	public function getVisitInfo($condition)
	{
		...
		$cache = Cache::tag("visit_info" . $condition["date"])->get("visit_info_" . $site_id . "_" . $condition["type"] . "_" . $condition["module"] . "_" . $condition["addon"] . "_" . $condition["date"]);
		if (!empty($cache)) {
	...
	{
		$site_id = isset($condition["site_id"]) ? $condition["site_id"] : 0;
		$cache = Cache::tag("visit_ip_info" . $condition["date"])->get("visit_ip_info_" . $site_id . "_" . $condition["type"] . "_" . $condition["module"] . "_" . $condition["addon"] . "_" . $condition["ip"] . "_" . $condition["date"]);
		if (!empty($cache)) {
			return success($cache);
		}
	...
	public function getVisitUserInfo($condition)
	{
		...
		$site_id = isset($condition["site_id"]) ? $condition["site_id"] : 0;
		$cache = Cache::tag("visit_user_info" . $condition["site_id"])->get("visit_user_info_" . $site_id . "_" . $condition["uid"] . "_" . $condition["module"] . "_" . $condition["addon"]);
		if (!empty($cache)) {
			return success($cache);
		}
	...
	public function getVisitCountData($param)
	{
		...
		switch ($date_type) {
			case 'today':
				...
				break;
			case 'yesterday':
				$date_condition = date('Ymd', strtotime('-1 days'));
				break;
			case 'week':
				$date_condition = [ "between", [ date('Ymd', strtotime('-6 days')), $today_date ] ];
				break;
			case 'month':
				$date_condition = [ "between", [ date('Ymd', strtotime('-29 days')), $today_date ] ];
				break;
			case 'daterange':
...
//		$visit_ip_count = $visit_model->getSum($condition, "ip_count");
		...
		//        $data = $visit_model->getInfo($condition, "sum(count) as visit_count,sum(ip_count) as visit_ip_count");
		return success($data);
