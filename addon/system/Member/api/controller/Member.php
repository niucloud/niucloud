...
// +---------------------------------------------------------------------+
namespace addon\system\Member\api\controller;
...
//		$member_info = api("System.Member.memberInfo", [ 'access_token' => $params['access_token'] ]);
	 ...
	 * @param $params
	 * @return \app\common\model\unknown|array
	 */
	 ...
	 * 创建交易流水号
	 */
	protected function createOutTradeNo()
	{
	 ...
	 * 获取站点会员相关数据
	 */
	public function getMemberRelate($params)
	{
		$list = hook('getMemberRelate', []);
		return success($list);
	}
	...
	public function initMemberInfo($params)
	{
		if (!empty($params['access_token'])) {
			...
			if ($member_info['code'] == 0) {
				cache("member_info_" . $params['site_id'] . $params['access_token'], $member_info);
			}
		}
	}
	 ...
	 * @param $params
	 */
	public function getSystemTacticsConfig($params)
	{
		$member_model = new MemberModel();
		$res = $member_model->getSystemTacticsConfig($params["site_id"]);
		return $res;
	}
