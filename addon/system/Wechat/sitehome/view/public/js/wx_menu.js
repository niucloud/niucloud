...
var menu = new Vue({
	...
	methods: {
		...
		addMaterial: function (type) {
			switch (type) {
				case 1 :
					window.open(nc.url("Wechat://sitehome/material/addgraphicmessage"));
					break;
				case 2 :
					uploadSingle();
					break;
		...
		loadMenu: function () {
			var _self = this;
			$.ajax({
				url: nc.url('Wechat://sitehome/menu/index'),
				data: {},
				dataType: "JSON",
				success: function (res) {
					...
					try {
						...
						if (data = JSON.parse(res.value)) {
							if (data.button == undefined || data.button == '' || data.button == null) {
								return false;
							}
							console.log(data.button[0]);
							_self.button = data.button;
							_self.menuIndex[-1, -1];
						}
					} catch (e) {
						console.log(e);
		...
		saveMenu: function () {
			...
			$.ajax({
				url: nc.url("Wechat://sitehome/menu/edit"),
		...
		$.ajax({
			...
			success: function (res) {
				...
				if (res.code == 0) {
					...
					menu.setValue('picurl', nc.img(_data.path));
					menu.picurl = nc.img(_data.path);
				}
				return true;
			}
		})
