...
function initVue() {
	...
	vue_obj = new Vue({
		...
		mounted: function () {
			...
			_self.editor = new Editor("editor", {}, function () {
				var index = _self.current_msg_index;
				var content = _self.editor.getContent();
				console.log(index, content);
				_self.article_item_list[index].content = content;
			});
		},
		methods: {
			// 添加图文消息
			...
			loadGraphicMessageInfo: function () {
				...
				$.ajax({
					type: 'post',
					url: nc.url("Wechat://sitehome/material/detail"),
					data: {id},
					dataType: "JSON",
			...
			editGraphicMessage: function () {
				...
				$.ajax({
					type: 'post',
					url: nc.url('Wechat://sitehome/material/edit'),
					data: {id, value},
					dataType: "JSON",
					success: function (res) {
						layer.msg(res.message);
						if (res.code == 0) {
							location.href = nc.url('Wechat://sitehome/material/index');
						} else {
							repeat_flag = false;
						}
					}
 ...
 * @param name
 */
function ablumUploadSuccess(data, name) {
	if (data.length != undefined && data.length > 0) {
		if (data.length > 1) {
			layer.msg('封面图只可选择一张');
		}
		path = nc.img(data[0].path);
