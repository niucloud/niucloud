...
WxReplay.prototype.getData = function (d) {
	...
	_this.sendAjax({
		url: nc.url('Wechat://sitehome/replay/index'),
		async: false,
...
WxReplay.prototype.clickEvent = {
	...
	delRule: function (d) {
		...
		var index = layer.open({
			...
			, yes: function (index, layero) {
				...
				flag = false;
				_this.sendAjax({
					url: nc.url('Wechat://sitehome/replay/delRule'),
	...
	delKeywords: function (d) {
		...
		var index = layer.open({
			...
			, yes: function (index, layero) {
				...
				flag = false;
				_this.sendAjax({
					url: nc.url('Wechat://sitehome/replay/delKeywords'),
	...
	editReply: function (d) {
		...
		if (request_type == 'image') {
			...
			active_pic += '<a class="picture" target="_blank" href=""><img src="' + nc.img(reply_content) + '" alt=""/></a>';
			active_pic += '</div>';
	...
	delReply: function (d) {
		...
		var index = layer.open({
			...
			, yes: function (index, layero) {
				...
				flag = false;
				_this.sendAjax({
					url: nc.url('Wechat://sitehome/replay/delReply'),
					data: {"rule_id": rule_id, "key_id": key_id},
					success: function (res) {
						layer.msg(res.message);
						if (res.code == 0) {
			...
			if (d.replay_list.length > 0) {
				for (j in d.replay_list) {
					...
					if (d.replay_list[j].type == 'text') {
						...
						html += '<span class="label"><img src="' + nc.img(d.replay_list[j].reply_content) + '" alt=""/></span>';
