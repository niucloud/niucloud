...
//		return success();//测试用
	...
	public function diy($param = [])
	{
		...
		if (!empty($diy_view_info['addon_name'])) {
			...
			$class_name = "\\addon\\system\\" . $diy_view_info['addon_name'] . "\\component\\controller\\Design";
			if (!class_exists($class_name)) {
				$class_name = "\\addon\\module\\" . $diy_view_info['addon_name'] . "\\component\\controller\\Design";
			}
	...
	public function diyUtils($param = [])
	{
		if (!empty($param['controller'])) {
			$class_name = '\\addon\\system\\' . $param['addon_name'] . '\\component\\controller\\' . $param['controller'];
			$is_exist = false;
			if (!class_exists($class_name)) {
				$class_name = '\\addon\\module\\' . $param['addon_name'] . '\\component\\controller\\' . $param['controller'];
				if (class_exists($class_name)) {
					$is_exist = true;
				}
			} else {
