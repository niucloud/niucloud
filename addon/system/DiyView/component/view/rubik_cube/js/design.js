...
var rubikCubeHtml = '<div class="layui-form-item">';
	...
	rubikCubeHtml += '<div class="selected-template-list layui-form-item">';
		...
		rubikCubeHtml += '<div class="layui-input-block">';
			rubikCubeHtml += '<ul>';
				rubikCubeHtml += '<li v-for="(item,i) in templateList" v-bind:class="[\'nc-border-color-selected\', (item.className == $parent.data.selectedTemplate) ?  \'selected\' : \'\' ]" v-on:click="changeTemplateList(i)">';
	...
	rubikCubeHtml += '<div class="layui-form-item">';
		...
		rubikCubeHtml += '<div class="layui-input-block layout">';
		
			rubikCubeHtml += '<ul v-for="item in templateList" v-if="(item.className == $parent.data.selectedTemplate) && !isShowCustomRubikCube">';
				rubikCubeHtml += '<li v-for="(li,i) in item.dimensionScale" v-bind:class="[item.className,(currentIndex==i) ? \'selected\' : \'\']" v-on:click="changeCurrentIndex(i)">';
					rubikCubeHtml += '<div class="empty" v-show="!currentList[i].imageUrl" v-html="li"></div>';
					rubikCubeHtml += '<div class="have-preview-image" v-show="currentList[i].imageUrl">';
			...
			rubikCubeHtml += '<ul v-if="isShowCustomRubikCube">';
				...
				rubikCubeHtml += '<li ref="rubikCube" class="custom-rubik-cube" v-for="(item,i) in customRubikCubeArray" v-bind:class="{ \'selected\' : item.selected,\'border-left\': (i%customRubikCube==0), \'border-bottom\' : (item.row == customRubikCube ) }" v-on:click="customRubikCubeClick(i)" v-on:mousemove="customRubikCubeMouseMove(i)" v-bind:style="{ width : (100/parseInt(customRubikCube)) + \'%\', height : $parent.data.heightArray[customRubikCube-4], lineHeight : $parent.data.heightArray[customRubikCube-4] }">';
			...
			rubikCubeHtml += '<template v-if="isShowCustomRubikCube &&currentList.length>0">';
				...
				rubikCubeHtml += '<div ref="selectedRubikCube" v-for="(item,i) in selectedRubikCubeArray" class="selected-rubik-cube" v-bind:style="{ top : item.top,left : item.left, width : item.width, height : item.height, lineHeight : item.height + \'px\', zIndex:(item.selected)?1:0, position : \'absolute\',textAlign : \'center\' }" v-bind:class="{ \'selected\' : item.selected, \'have-image\' : currentList[i].imageUrl }"';
				...
				rubikCubeHtml += ' >';
					...
					rubikCubeHtml += '<i class="del" v-on:click.stop="deleteSelectedRubikCubeClick(i)">x</i>';
		...
		rubikCubeHtml += '<p>选定布局区域，在下方添加图片，建议添加比例一致的图片</p>';

		rubikCubeHtml += '<slide v-bind:data="{ field : \'imageGap\', label : \'图片间隙\' }"></slide>';
		// rubikCubeHtml += '<div class="layui-form-item">';
		// 	rubikCubeHtml += '<label class="layui-form-label sm">图片间隙</label>';
		// 	rubikCubeHtml += '<div class="layui-input-block">';
		// 		rubikCubeHtml += '<input type="range" maxlength="30" v-model="$parent.data.imageGap" />';
		// 		rubikCubeHtml += '<span>{{$parent.data.imageGap}}</span>';
		// 	rubikCubeHtml += '</div>';
		// rubikCubeHtml += '</div>';
			...
			rubikCubeHtml += '<ul>';
				rubikCubeHtml += '<li>';
					...
					rubikCubeHtml += '<div class="content-block">';
						rubikCubeHtml += '<nc-link v-bind:data="{ field : $parent.data.list[currentIndex].link }" v-bind:callback="linkCallBack"></nc-link>';
					rubikCubeHtml += '</div>';
				rubikCubeHtml += '</li>';
...
//			console.log(JSON.stringify(this.customRubikCubeArray));
		...
		initTemplateList : function(){
			...
			var prefix = RESOURCEPATH + "/component/view/rubik_cube/img";
			this.templateList.push({ name : "1行2个", src :  prefix + "/rubik_cube_row1_of2.png", className : "row1-of2", dimensionScale : ["宽度375像素<br/>或同等比例","宽度375像素<br/>或同等比例"] });
			this.templateList.push({ name : "1行3个", src : prefix + "/rubik_cube_row1_of3.png", className : "row1-of3", dimensionScale : ["宽度250像素<br/>或同等比例","宽度250像素<br/>或同等比例","宽度250像素<br/>或同等比例"] });
			this.templateList.push({ name : "1行4个", src : prefix + "/rubik_cube_row1_of4.png", className : "row1-of4", dimensionScale : ["宽度188像素<br/>或同等比例","宽度188像素<br/>或同等比例","宽度188像素<br/>或同等比例","宽度188像素<br/>或同等比例"] });
			this.templateList.push({ name : "2左2右", src : prefix + "/rubik_cube_row2_lt_of2_rt.png", className : "row2-lt-of2-rt", dimensionScale : ["375x375像素<br/>或同等比例","375x375像素<br/>或同等比例","375x375像素<br/>或同等比例","375x375像素<br/>或同等比例"] });
			this.templateList.push({ name : "1左2右", src : prefix + "/rubik_cube_row1_lt_of2_rt.png", className : "row1-lt-of2-rt", dimensionScale : ["375x750像素<br/>或同等比例","375x375像素<br/>或同等比例","375x375像素<br/>或同等比例"] });
			this.templateList.push({ name : "1上2下", src : prefix + "/rubik_cube_row1_tp_of2_bm.png", className : "row1-tp-of2-bm", dimensionScale : ["750x375像素<br/>或同等比例","375x375像素<br/>或同等比例","375x375像素<br/>或同等比例"] });
			this.templateList.push({ name : "1左3右", src : prefix + "/rubik_cube_row1_lt_of1_tp_of2_bm.png", className : "row1-lt-of1-tp-of2-bm",dimensionScale : ["375x750像素<br/>或同等比例","375x375像素<br/>或同等比例","188x375像素<br/>或同等比例","188x375像素<br/>或同等比例"] });
