...
hui.tab = function (selector) {
	...
	for (var i = 0; i < tabLists.length; i++) {
		var tabRun = new huiTabListBase(tabLists.dom[i]);
	}
}
huiTabListBase = function (dom) {
	...
	this.swipeIn.swipe(function (e) {
		...
		// 	this.isMove = false;
		// 	return false;
		// }else if (_self.index >= _self.itemSize - 1 && e.deltaX < 0) {
		// 	this.isMove = false;
		// 	return false;
	...
	this.changeTo = function () {
		...
		_self.swipeIn.dom[0].style.height = hui(_self.items.dom[_self.index]).height() + "px";
		setTimeout(function () {
